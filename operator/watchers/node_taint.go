...
func checkAndMarkNode(c kubernetes.Interface, nodeGetter slimNodeGetter, nodeName string, options markNodeOptions) bool {
	...
	if (options.RemoveNodeTaint && hasAgentNotReadyTaint(node)) ||
		...
		if isCiliumPodRunning(node.GetName()) {
			...
			log.WithFields(logrus.Fields{"node-name": node.GetName()}).Debug("Cilium pod not running for node")
		}
	} else {
		log.WithFields(logrus.Fields{"node-name": node.GetName()}).Debug("Node without taint and with CiliumIsUp condition")
	}
	return true
}
